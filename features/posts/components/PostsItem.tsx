      ...
      >
        <div className="flex justify-between items-start mb-3 gap-1">
          ...
          {props.manage && props.is_published ? (
            <span className="inline-block px-2 py-1 text-xs bg-indigo-500 text-white rounded-full shrink-0">
              公開中
            </span>
          ) : props.manage && !props.is_published ? (
            <span className="inline-block px-2 py-1 text-xs bg-gray-500 text-white rounded-full shrink-0">
              非公開
            </span>
          ) : null}
        </div>
